...
-- 条件查询
	...
	-- 模糊查询
		...
		-- _ 替换1个
		-- 查询姓名中 以 "小" 开始的名字
		select name from students where name="小";  # 精准匹配小字，并不是以小开头的
	...
	-- 范围查询
		...
		select * from students where not age between 18 and 34; --不推荐
...
-- 排序
	...
	select * from students where (age between 18 and 34) and gender=1 order by age asc; --asc缺省
...
-- 分组
	...
	-- group by
	-- 按照性别分组,查询所有的性别
	select name from students group by gender; --有时会错误，有时会默认将分组中的第一个代表拿出来。
	select * from students group by gender; --有时会错误，有时会默认将分组中的第一个代表拿出来。
	...
	-- group_concat(...)  # 写上哪个字段，那么就会以一个字段的形式将其都展示出来
	-- 查询同种性别中的姓名
...
-- 连接查询
	...
	select * from students inner join classes;  -- 数据会乱
	...
	-- 当是同一个班级的时候，按照学生的id进行从小到大排序（排序条件可以多个，依次按照条件进行排序）
	...
	select * from students as s left join classes as c on s.cls_id=c.id having c.id is null; -- 此时推荐having
	select * from students as s left join classes as c on s.cls_id=c.id where c.id is null;

	-- right join   on
	-- 将数据表名字互换位置，用left join完成

	-- inner 两个表匹配的数据 left 两个表匹配的数据，对于右边不存在的数据，会补充为null） right参考左边即可
