...
-- 读已经提交的（其它数据库的默认级别）：
	...
	- 这样的话就会存在不可重复读和幻读情况。
-- 可重复读（默认级别）
	- 在一个事物中，另一个事物的数据修改和本次事物没有关系，可以重复的读取上次读取的数据，没有不可重复读的情况，
	- 如果要写数据的话，那么就必须等到别的事物结束了写事物。
	- 但是对于之前读取的行数如果修改了，还是会读取修改行的，这便是还存在幻读情况。
--  可串行化
	- 会强制事物排序，每个事物互不干扰，一个事物如果不结束的话，另外一个事物不允许操作该条数据或者该表数据，
	- 所以也就不存在两个事物同时更新一条或者插入一条的情况了，
	- 但是容易造成锁的竞争，进而超时。
...
-- 补充：
	...
	- 就要等待读锁的释放，如果另外一个事物也要更新数据的话，那么就会出现死锁现象
